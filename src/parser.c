...
bool parse_basetype(ParserState *ps) {
    ...
    if (obj) {
        kv_pushobj(ps->const_val, obj);
        kv_pushbc(ps->info->opcodes, BC_LOADCONST);
        kv_pushbc(ps->info->opcodes, kv_size(ps->const_val));
        return true;
    } else {
        ret = parse_mutabletype(ps, &times);
        if (ret) {
            // NEW_OBJ TYPE EXTRA
            kv_pushbc(ps->info->opcodes, BC_NEW_OBJ_EXTRA);
            kv_pushbc(ps->info->opcodes, ret);
            kv_pushbc(ps->info->opcodes, times);
        }
        return ret != 0;
    }
}
...
bool parse_try_t(ParserState *ps) {
    ...
    switch (tk->val) {
        case TK_NAME:
            kv_pushbc(ps->info->opcodes, BC_LOAD_VAL);
            kv_pushbc(ps->info->opcodes, (uintptr_t)tk->obj);
            next(ps);
            break;
        case '(':
            next(ps);
        ...
        case '+': case '-':
            ...
            parse_expr10(ps);
            print_tk_val(tk_val);
            kv_pushbc(ps->info->opcodes, BC_OPERATOR);
            kv_pushbc(ps->info->opcodes, tk_val == '+' ? OP_POS : OP_NEG);
            break;
        case '~':
            next(ps);
            parse_expr10(ps);
            print_tk_val('~');
            kv_pushbc(ps->info->opcodes, BC_OPERATOR);
            kv_pushbc(ps->info->opcodes, token_to_op_val('~'));
            break;
        case TK_KW_NOT:
            next(ps);
            parse_expr10(ps);
            ...
            kv_pushbc(ps->info->opcodes, BC_OPERATOR);
            kv_pushbc(ps->info->opcodes, token_to_op_val(TK_KW_NOT));
            break;
        default:
...
static _INLINE bool parse_try_index(ParserState *ps) {
    if (ps->ls->token.val == '[') {
        ...
        parse_expr(ps);
        ACCEPT(ps, ']');
        kv_pushbc(ps->info->opcodes, BC_GET_ITEM);
        return true;
    }
    return false;
}
...
_INLINE void parse_expr1(ParserState *ps) {
    ...
    switch (tk->val) {
        case TK_KW_OR:
            ...
            kv_pushbc(ps->info->opcodes, BC_OPERATOR);
            kv_pushbc(ps->info->opcodes, OP_OR);
            parse_expr1(ps);
            break;
    }
}
...
_INLINE void parse_expr2(ParserState *ps) {
    ...
    switch (tk->val) {
        case TK_KW_AND:
            ...
            kv_pushbc(ps->info->opcodes, BC_OPERATOR);
            kv_pushbc(ps->info->opcodes, OP_AND);
            parse_expr2(ps);
            break;
    }
}
...
success:
    ...
    kv_pushbc(ps->info->opcodes, BC_OPERATOR);
    kv_pushbc(ps->info->opcodes, op_val);
    parse_expr3(ps);
}
...
_INLINE void parse_expr4(ParserState *ps) {
    ...
    switch (tk->val) {
        case '|':
            ...
            print_tk_val('|');
            kv_pushbc(ps->info->opcodes, BC_OPERATOR);
            kv_pushbc(ps->info->opcodes, OP_BITOR);
            parse_expr4(ps);
            break;
    }
}
...
_INLINE void parse_expr5(ParserState *ps) {
    ...
    switch (tk->val) {
        case '^':
            ...
            print_tk_val('^');
            kv_pushbc(ps->info->opcodes, BC_OPERATOR);
            kv_pushbc(ps->info->opcodes, OP_BITXOR);
            parse_expr5(ps);
            break;
    }
}
...
_INLINE void parse_expr6(ParserState *ps) {
    ...
    switch (tk->val) {
        case '&':
            ...
            print_tk_val('&');
            kv_pushbc(ps->info->opcodes, BC_OPERATOR);
            kv_pushbc(ps->info->opcodes, OP_BITAND);
            parse_expr6(ps);
            break;
    }
}
...
_INLINE void parse_expr7(ParserState *ps) {
    ...
    switch (tk->val) {
        case TK_OP_LSHIFT: case TK_OP_RSHIFT:
            ...
            print_tk_val(tk_val);
            kv_pushbc(ps->info->opcodes, BC_OPERATOR);
            kv_pushbc(ps->info->opcodes, token_to_op_val(tk_val));
            parse_expr7(ps);
            break;
    }
}
...
_INLINE bool parse_try_expr10(ParserState *ps) {
    ...
    switch (tk->val) {
        case TK_OP_POW:
            ...
            kv_pushbc(ps->info->opcodes, BC_OPERATOR);
            kv_pushbc(ps->info->opcodes, OP_POW);
            break;
    }
    return true;
}
...
void parse_single_left_value(ParserState *ps) {
    ...
    if (tk->val == TK_NAME) {
        ...
        if (tk->val == '[') {
            ...
            if (tk->val == '=') {
                kv_pop(ps->info->opcodes);
                next(ps);
                parse_expr(ps);
                //kv_pushbc(ps->info->opcodes, BC_SET_ITEM);
                //kv_pushbc(ps->info->opcodes, (uintptr_t)obj);
            }
        }
    }
}
...
void loop_control_replace(ParserState *ps, int start_pos) {
    for (unsigned int i = start_pos; i < kv_size(ps->info->opcodes);) {
        switch (kv_A(ps->info->opcodes, i)) {
            ...
            case BC_DEL_FORCE:
                i += 1; break;
            case BC_FAKE_BREAK:
                if (kv_A(ps->info->opcodes, i + 1) == ps->info->loop_depth + 1) {
                    kv_A(ps->info->opcodes, i) = BC_JMP;
                    kv_A(ps->info->opcodes, i + 1) = kv_size(ps->info->opcodes) - i;
                }
                break;
            case BC_FAKE_CONTINUE:
                if (kv_A(ps->info->opcodes, i + 1) == ps->info->loop_depth + 1) {
                    kv_A(ps->info->opcodes, i) = BC_JMP;
                    kv_A(ps->info->opcodes, i + 1) = kv_size(ps->info->opcodes) - i - 2;
                }
                break;
            default: i += 2; break;
        }
...
void parse_stmt(ParserState *ps) {
    ...
    switch (tk->val) {
        case TK_NAME:
            ...
            switch (tk->val) {
                case '=':
                    next(ps);
                    parse_expr(ps);
                    kv_pushbc(ps->info->opcodes, BC_SET_VAL);
                    kv_pushbc(ps->info->opcodes, (uintptr_t)obj);
                    break;
                ...
                case '(':
                    ...
                    kv_pushbc(ps->info->opcodes, BC_CALL);
                    kv_pushbc(ps->info->opcodes, (uintptr_t)tmp);
                    kv_pushbc(ps->info->opcodes, (uintptr_t)obj);
                    ACCEPT(ps, ')');
                    break;
                case '[':
        ...
        case TK_KW_IF:
            ...
            parse_expr(ps);
            ACCEPT(ps, ':');
            kv_pushbc(ps->info->opcodes, BC_TEST);
            kv_pushbc(ps->info->opcodes, 0);
            tmp = tmp3 = kv_size(ps->info->opcodes);
            ...
            kv_A(ps->info->opcodes, tmp - 1) = kv_size(ps->info->opcodes) - tmp;
            ...
            while (tk->val == TK_KW_ELIF) {
                ...
                // jmp X
                kv_A(ps->info->opcodes, tmp - 1) += 2;
                kv_pushbc(ps->info->opcodes, BC_JMP);
                kv_pushbc(ps->info->opcodes, 0);
                tmp2 = kv_size(ps->info->opcodes);
                ...
                // write X for jmp X
                kv_A(ps->info->opcodes, tmp2 - 1) = kv_size(ps->info->opcodes) - tmp2 + 2;
                // write X for test X
                kv_A(ps->info->opcodes, tmp - 1) = kv_size(ps->info->opcodes) - tmp;
            }
            ...
            // new code pos
            final_pos = kv_size(ps->info->opcodes);
            // tmp3 <- pos of first jmp (if exists)
            tmp3 += kv_A(ps->info->opcodes, tmp3 - 1);

            // change every jmp point
            while (tmp3 != final_pos) {
                // tmp2 <- pos of next jmp (if exists) or block end
                tmp2 = tmp3 + kv_A(ps->info->opcodes, tmp3 - 1);
                kv_A(ps->info->opcodes, tmp3 - 1) = final_pos - tmp3;
                tmp3 = tmp2;
            }
            return;
        case TK_KW_WHILE:
            ...
            kv_pushbc(ps->info->opcodes, BC_JMP_BACK);
            kv_pushbc(ps->info->opcodes, (kv_size(ps->info->opcodes) - tmp + 1));
            kv_A(ps->info->opcodes, tmp2 - 1) = kv_size(ps->info->opcodes) - tmp2;
            return;
        case TK_KW_BREAK:
            next(ps);
            if (ps->info->loop_depth == 0) error(ps, PYLT_ERR_PARSER_BREAK_OUTSIDE_LOOP);
            else {
                kv_pushbc(ps->info->opcodes, BC_FAKE_BREAK);
                kv_pushbc(ps->info->opcodes, ps->info->loop_depth);
            }
            break;
        case TK_KW_CONTINUE:
            next(ps);
            if (ps->info->loop_depth == 0) error(ps, PYLT_ERR_PARSER_CONTINUE_OUTSIDE_LOOP);
                ...
                kv_pushbc(ps->info->opcodes, BC_FAKE_CONTINUE);
                kv_pushbc(ps->info->opcodes, ps->info->loop_depth);
            }
            break;
        case TK_KW_FOR:
            ...
            if (tk->val == TK_NAME) {
                ...
                parse_expr(ps);
                ACCEPT(ps, ':');
                kv_pushbc(ps->info->opcodes, BC_NEW_OBJ);
                kv_pushbc(ps->info->opcodes, PYLT_OBJ_TYPE_ITER);
                tmp = kv_size(ps->info->opcodes);
                kv_pushbc(ps->info->opcodes, BC_FORITER);
                kv_pushbc(ps->info->opcodes, 0);
                kv_pushbc(ps->info->opcodes, BC_SET_VAL);
                kv_pushbc(ps->info->opcodes, (uintptr_t)obj);
                ...
                kv_A(ps->info->opcodes, tmp + 1) = kv_size(ps->info->opcodes) - tmp + 1;
                kv_pushbc(ps->info->opcodes, BC_JMP_BACK);
                kv_pushbc(ps->info->opcodes, (kv_size(ps->info->opcodes) - tmp + 1));

                kv_pushbc(ps->info->opcodes, BC_DEL_FORCE);
            } else error(ps, PYLT_ERR_PARSER_INVALID_SYNTAX);
            return;
        case TK_KW_PASS:
            next(ps);
        ...
        default:
            parse_expr(ps);
            kv_pushbc(ps->info->opcodes, BC_POP);
    }
    ACCEPT(ps, TK_NEWLINE);
}
...
void parse(ParserState *ps) {
    ...
    //parse_expr(ps);
    parse_stmts(ps);
    kv_pushbc(ps->info->opcodes, BC_PRINT);
}
...
void func_push(ParserState *ps) {
    ...
    if (!ps->info_used) {
        ...
        kv_init(info->opcodes);
    // 如果没有空槽
    } else {
        info = ps->info_used;
        ps->info_used = ps->info_used->prev;
        kv_clear(info->opcodes);
    }
